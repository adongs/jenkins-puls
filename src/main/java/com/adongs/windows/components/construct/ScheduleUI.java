 ...
 * @author yudong
 * @version 1.0
 * @date 2020/10/7 8:28 下午
 * @modified By
 */
public class ScheduleUI extends BasicProgressBarUI {
    ...
    protected void paintDeterminate(Graphics g, JComponent c) {
        ...
        try {
            ...
            if (orientation == SwingConstants.HORIZONTAL) {
                ...
                fullShape = getShapedRect(r.x, yOffset, r.width, pHeight-1, 0);
                coloredShape = getShapedRect(r.x, yOffset, amountFull, pHeight-1, 0);
            }
            else {
                int pWidth = progressBar.getPreferredSize().width;
                int xOffset = r.x + (r.width - pWidth) / 2;

                fullShape = getShapedRect(xOffset, r.y, pWidth, r.height, 0);
                coloredShape = getShapedRect(xOffset, r.y, pWidth, amountFull, 0);
            }
